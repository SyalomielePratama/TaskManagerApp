...
class PasswordResetRequestSerializer(serializers.Serializer):
    ...
    def validate_email(self, value):
        if not User.objects.filter(email=value).exists():
            raise serializers.ValidationError("Tidak ada user dengan email ini.")
        return value
    ...
    def save(self):
        ...
        # TODO: Ganti 'your-frontend-url' dengan URL frontend Anda untuk reset password
        reset_url = f"http://your-frontend-url/reset-password/{uid}/{token}/" 
        
        # Kirim email
        # Ini adalah implementasi dasar, Anda mungkin ingin menggunakan template email HTML
        try:
            send_mail(
                'Reset Password Akun Task Manager Anda',
                f'Halo {user.username},\n\n'
                f'Silakan klik link berikut untuk mereset password Anda:\n{reset_url}\n\n'
            ...
            print(f"Link reset password dikirim ke {user.email}: {reset_url}") # Untuk debug di console
        except Exception as e:
            # Log error jika pengiriman email gagal
            print(f"Error sending password reset email: {e}")
            raise serializers.ValidationError("Gagal mengirim email reset password. Coba lagi nanti.")
...
class PasswordResetConfirmSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        try:
            ...
            raise serializers.ValidationError("Link reset password tidak valid atau sudah kedaluwarsa.")

        if not PasswordResetTokenGenerator().check_token(self.user, attrs['token']):
            raise serializers.ValidationError("Token reset password tidak valid atau sudah kedaluwarsa.")

        if attrs['new_password'] != attrs['new_konfirmasi_password']:
            raise serializers.ValidationError({"new_password": "Password baru tidak cocok."})
        
        # Anda bisa menambahkan validasi kekuatan password di sini
        return attrs
